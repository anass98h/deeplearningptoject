...
async def process_video(file: UploadFile = File(...)):
    """
    Endpoint to process a video file and extract pose data.
    Simply waits for a couple seconds and returns success.
    """
    try:
        # Log the received file
        logging.info(f"Received video file: {file.filename}, size: {file.size} bytes")
...
@app.get("/video-data/original")
async def get_original_video_data():
    """
    Return the original pose data from a video.
    Uses the existing A1_kinect.csv file as a placeholder.
    """
    try:
        # Path to the CSV file (using the same file as kinect-data for demonstration)
        csv_path = Path("./A1_kinect.csv")
        ...
        logging.error(f"Error serving original video data: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get original data: {str(e)}",
            headers={"error_type": "data_retrieval_error"}
        )

@app.get("/video-data/trimmed")
async def get_trimmed_video_data():
    """
    Return the trimmed pose data from a video.
    Uses the existing output.csv file as a placeholder.
    """
    try:
        # Path to the CSV file (using the same file as trim-frames for demonstration)
        csv_path = Path("./tests/output.csv")
        ...
        if not csv_path.exists():
            raise HTTPException(
                status_code=404,
                detail="Trimmed CSV file not found",
                headers={"error_type": "file_not_found"}
            )
        ...
        logging.error(f"Error serving trimmed video data: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get trimmed data: {str(e)}",
            headers={"error_type": "data_retrieval_error"}
        )